...
TestSndCnt::TestSndCnt(int appId, int myRcvNum): TestAppInterface(appId),
...
//			abort();
		...
		*stamp = counter;
		...
		int dstAppId = RECEIVER + rand() % rcvNum;
		if(!proxy.send(stamp, dstAppId))
		{
...
//	int coreId = atoi(argv[2]);
	int coreId = appId;
	int rcvNum = atoi(argv[2]);

	signal(SIGUSR1, stopSig);

	tester = new TestSndCnt(appId, rcvNum);
